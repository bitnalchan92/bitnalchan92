...
function updateForm(num) {
    $('.comment-tool, .LayerMore').hide(); // 더보기 및 수정 삭제 역역 숨김
...
function replyform(num, lev, seq, ref) {
    ...
    // 답글 폼의 '.btn-register'에 클래스 'reply'를 추가합니다.
...
$(function () {
    ...
    $('.comment-area').on('click', '.reply-cancel', function () {
        ...
        $('.comment-tool').show(); // 더 보기 영역이 보이도록 합니다.
    ...
    $('.comment-area').on('click', '.comment-info-button', function (event) {
        ...
        if (length == 1) {
            // 답글쓰기 폼이 한 개가 존재하면 anchor 태그(<a>)의 기본 이벤트를 막아 또 다른 답글쓰기 폼이 나타나지 않도록 합니다.
